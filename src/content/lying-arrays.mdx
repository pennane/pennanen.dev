...
So, instead of defining players as just an array, we can go ham, and enforce the minimum player count already on type level.
...
Nothing is keeping from going further than 2 either - or making the type generic.
...
type AtLeast3Array<T> = NonEmptyArray<T> & { third: T }
```
