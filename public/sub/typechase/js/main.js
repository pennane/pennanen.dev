;(async () => {
  ...
  // Mobile check
  document
    .getElementById('mobile-textarea')
  ...
  const textsPath =
    window.location.host === 'https://pennanen.dev'
      ? 'https://pennanen.dev/projects/typechase/assets/texts.json'
      : './assets/texts.json'
    ...
    ;[...document.querySelectorAll('.skilltablebody')].forEach(destination => {
      ...
      skillLevels.forEach(skill => {
        ...
        content.textContent =
          ...
          skill.lowest +
          '-' +
          (skill.highest.toString().match(/^\d+$/) ? skill.highest : '') +
          ' wpm'
    ...
    text.words.forEach((word, j) => {
      ...
      word.characters.forEach((char, k) => {
        ...
        charElement.setAttribute(
          'class',
          `slicedChar ${classtag}${i} ${
            charAtIndex.match(/\ /) ? 'slicedSpace' : null
          }`
        )
      ...
      if (j + 1 < text.words.length) {
        ...
        spaceCharElement.setAttribute(
          'class',
          `slicedChar ${classtag}${i} slicedSpace`
        )
  ...
  function calculateMissedCharacters(textInstance) {
    return textInstance.text.characters.reduce(
      (accumulated, current) =>
        current.failed ? accumulated + 1 : accumulated,
      0
    )
  }
  ...
  function calculateMissingCharacters(textInstance) {
    return textInstance.text.characters.reduce(
      (accumulated, current) =>
        !current.typed ? accumulated + 1 : accumulated,
      0
    )
  }
  ...
  function getWPM(textInstance) {
    return parseInt(
      textInstance.characterindex /
        5 /
        ((Date.now() - textInstance.timing) * 0.0000166666)
    )
  }
  ...
  function completedChase() {
    ...
    if (
      textInstance.stats.accuracy > 25 &&
      textInstance.stats.wpm > 10 &&
      missing <= 1
    ) {
      ...
      popup(
        'Some characters were left in incorrectly. Score not saved.',
        '#b94400',
        4200
      )
    }
  ...
  function typingListener(event, textInstance) {
    ...
    if (
      [13, 8, 222, 160, 32].indexOf(event.keyCode) > -1 &&
      ['input', 'textarea'].indexOf(event.target.nodeName.toLowerCase()) === -1
    ) {
    ...
    if (key === keyToMatch.character) {
      ...
      characterindex === 0
        ? null
        : (keyElement = document.querySelector(
            `.${slicedClassTag}${characterindex - 1}`
          ))
    } else {
      keyState = 'incorrect'
  ...
  function createChaseElement(trimmedInstance) {
    let { hours, minutes, seconds, year, month, day } = sliceTime(
      trimmedInstance.timing
    )
    ...
    trimmedInstance.custom
      ? customValue.setAttribute('class', 'chaseCustomText')
      : customValue.setAttribute('class', 'chaseDefaultText')
    trimmedInstance.custom
      ? customValue.setAttribute('title', 'custom text')
      : customValue.setAttribute('title', 'default text')
  ...
  function chaseElementClick(event, storage) {
    ...
    if (
      targetClasslist.indexOf('removeChaseButton') > -1 ||
      parentClasslist.indexOf('removeChaseButton') > -1
    ) {
    ...
    navigator.clipboard
      .writeText(chase.querySelector('.chaseFullText').textContent)
      .catch(console.log)
  ...
  function updateLast10Stats(storage) {
    ...
    document.getElementById('last10WpmParent').className =
      'skillLevel_' + skill.level
  ...
  document
    .querySelector('#cancelcustomtext')
    .addEventListener('click', event => {
      openCustomTextMenu(false, textInstance)
    })
  ...
  document
    .querySelector('#engagecustomtext')
    .addEventListener('click', event => {
